...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    if not OTHER: # first time calling this function, set up my OTHER variables.
        ...
        OTHER = (kalman_filter_other) 
    else: # not the first time
        
        # call kalman filter
        predicted_measurement, OTHER = estimate_next_pos(target_measurement, OTHER)
        # distance between 
        predited_distance = distance_between(predicted_measurement, hunter_position)
        # gets the min distance, so that robot does not goes faster than needed
    ...
    return steer_control, vel_control, OTHER
...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if not OTHER:
        OTHER = ExtendedKalmanFilter()

    # gets current x and y measurements
    measure = matrix([
        [measurement[0]],
        [measurement[1]]
    ])
    ...
    OTHER.update(measure) #  update
    x, y = OTHER.predict() #  predict
    return (x, y), OTHER
