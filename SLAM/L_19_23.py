...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    # Initialize Omega and Xi (with both x and y positions)
    Omega = matrix()
    Xi = matrix()
    states_size = 2
    dimension = (N + num_landmarks)*states_size 
    
    Omega.zero(dimension, dimension) # x,y from number of states and landmarks
    Xi.zero(dimension ,1)
    
    # Initialize intial position contraints
    Omega.value[0][0] = 1 # x
    Omega.value[1][1] = 1 # y
    # beings in the middle of the map
    Xi.value[0][0] = world_size/2 # x
    Xi.value[1][0] = world_size/2 # y
    
    # loop over all data
    for k in range(len(data)):
        # data has measurements and motion information
        measurement = data[k][0]
        motion = data[k][1]
        
        n = k*states_size # motion index      
        
        # sensing
        for i in range(len(measurement)):
            m = states_size * (N + measurement[i][0]) # landmark index
            for b in range(states_size): # update x and y
                Omega.value[n+b][n+b] += 1.0 / measurement_noise # top left corner
                Omega.value[m+b][m+b] += 1.0 / measurement_noise # bottom right corner
                Omega.value[n+b][m+b] += -1.0 / measurement_noise # top right corner
                Omega.value[m+b][n+b] += -1.0 / measurement_noise # bottom left corner
                Xi.value[n+b][0] += -measurement[i][1+b] / measurement_noise # motion
                Xi.value[m+b][0] += measurement[i][1+b] / measurement_noise # landmark
        
        # motion
        for b in range(2*states_size): # diagonal of x and y are 1s
            Omega.value[n+b][n+b] += 1.0 / motion_noise
        for b in range(states_size): # x and y
            Omega.value[n+b][n+b+2] += -1.0 / motion_noise # top
            Omega.value[n+b+2][n+b] += -1.0 / motion_noise # bottom
