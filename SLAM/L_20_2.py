...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # n is the index of the robot pose in the matrix/vector
#        n = k * 2 
        n = 0 # no need anymore, since we update every single new entry
        ...
        # expand matrix for next x,y motion
        motion_idx = [0,1] # keep first motion as the first indexes
        map_idx = list(range(4,dim+2)) # change idxs from landmarks
        Omega = Omega.expand(dim + 2, dim + 2, motion_idx + map_idx, motion_idx + map_idx)
        Xi =    Xi.expand(dim + 2, 1, motion_idx + map_idx, [0])
        ...
        A = Omega.take([0,1],list(range(2,dim + 2)))
        B = Omega.take([0,1],[0,1])
        C = Xi.take([0,1],[0])
        Omega_prime = Omega.take(list(range(2,dim+2)),list(range(2,dim+2)))
        Xi_prime = Xi.take(list(range(2,dim+2)),[0])
