...
class robot:
    ...
    def move(self, motion): 
        ...
        distance = motion[1] # distance drive forward
        steering = motion[0] # steering motion of the car
        steering_noisy = random.gauss(steering, self.steering_noise)
        distance_noisy = random.gauss(distance, self.distance_noise)
        ...
        beta = (distance_noisy/self.length)*tan(steering_noisy) # turning angle
        # in case beta is small (robot goes straight forward)
        if abs(beta) < 0.001:
            # compute robots coordinates
            res.x = self.x + distance_noisy*cos(self.orientation)
            res.y = self.y + distance_noisy*sin(self.orientation)
            #self.orientation = fmod((self.orientation+beta), 2*pi)
            return res
        else:
            R = distance_noisy/beta # turning radius
            # compute point in which robot is turning
            cx = self.x - sin(self.orientation)*R
            cy = self.y + cos(self.orientation)*R
            # compute robots coordinates
            res.x = cx + sin(self.orientation + beta)*R
            res.y = cy - cos(self.orientation + beta)*R
            res.orientation = fmod((self.orientation+beta), 2*pi)
    ...
    def sense(self, noise = 1): 
        ...
        for i in range(len(landmarks)):
            # distance to landmark (notice that the landmark position is given as (y,x))
            dx = -self.x+landmarks[i][1] 
            dy = -self.y + landmarks[i][0]
            if noise:
                bearing_noisy = random.gauss(0.0, self.bearing_noise)
            else:
                bearing_noisy = 0.0
            bearing = atan2(dy,dx) - self.orientation + bearing_noisy#  gets bearing angle
            bearing %=2.0*pi # normalize
            Z.append(bearing)
            #print (landmarks[i][0])
